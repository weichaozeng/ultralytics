...
It expands the packed RGB bits through the same synthetic RGB->RGGB path used by
`det_qnns.py`, then visualizes `sum / chunk_size` over a temporal chunk.
"""
...
def _raw_mean_to_rgb(raw_mean: np.ndarray) -> np.ndarray:
    """Convert raw Bayer mean `(2H,2W)` to RGB `(H,W,3)`."""
    r = raw_mean[0::2, 0::2]
    g1 = raw_mean[0::2, 1::2]
    g2 = raw_mean[1::2, 0::2]
    b = raw_mean[1::2, 1::2]
...
def main() -> None:
    ...
    raw = packed_frames_to_raw_bayer(packed, expected_w=args.expected_w, ch_order=args.packed_ch_order)
    raw_mean = raw.astype(np.float32).sum(axis=0) / float(args.chunk_size)
    rgb_mean = _raw_mean_to_rgb(raw_mean)

    _print_stats("packed_chunk", packed)
    _print_stats("raw_mean", raw_mean)
