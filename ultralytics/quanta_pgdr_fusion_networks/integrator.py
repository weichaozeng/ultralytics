"""PG-DR path 1: Poisson-Gamma deviance with STEA-style chunk fusion."""
...
class PoissonGammaDevianceFusion(nn.Module):
    """
    PG-DR fusion path (path 1).

    Fast branch: causal conv1d over photon bins.
    Slow branch: persistent Gamma sufficient statistics (alpha_s, beta_s).
    Motion: Poisson deviance of fast-window counts vs chunk-entry slow rate.
    Fusion: STEA-style reverse cummax valid_weight blending with y_fast.
    """
...
class PoissonGammaDevianceFusionFrame(nn.Module):
    """Frame-mode wrapper that emits one reconstructed frame plus a confidence map."""

    def __init__(self, **kwargs):
        super().__init__()
        self.core = PoissonGammaDevianceFusion(**kwargs)
