...
"""PoseTrack variant that exports a dense Gaussian velocity field for SPAD preprocessors."""
...
from .pose_track import PoseSTrack, PoseTrack


class SPADPoseTrack(PoseTrack):
    """PoseTrack with dense velocity-field export for SPAD preprocessing."""

    def __init__(self, args: Any, frame_rate: int = 30, class_names: dict | list | None = None):
        super().__init__(args=args, frame_rate=frame_rate, class_names=class_names)
    ...
    def update(
        ...
        tracks = super().update(results, img=img, feats=feats, keypoints=keypoints)
    ...
    def _track_state_to_motion(self, track: PoseSTrack, frame_hw: tuple[int, int]) -> dict[str, float] | None:
        if track.mean is None:
            ...
            bw = max(float(track.mean[2]), 1.0)
            bh = max(float(track.mean[3]), 1.0)
            vx = float(track.mean[4])
            vy = float(track.mean[5])

        wrist = track.pred_keypoints_xy[0]
        cx = float(wrist[0])
        cy = float(wrist[1])

        if track.kpt_conf_ema is not None:
            score = float(track.score * max(track.kpt_conf_ema.mean(), 1e-3))
        elif track.keypoints.shape[1] > 2:
            score = float(track.score * max(track.keypoints[:, 2].mean(), 1e-3))
        else:
    ...
    def _render_velocity_field(self, frame_hw: tuple[int, int], motion: list[dict[str, float]]) -> torch.Tensor:
        ...
        if np.any(valid):
            field[..., 0][valid] = numer[..., 0][valid] / denom[valid]
            field[..., 1][valid] = numer[..., 1][valid] / denom[valid]
