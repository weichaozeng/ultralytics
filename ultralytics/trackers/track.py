...
from ultralytics.custom.posetracker import PoseTracker

# A mapping of tracker types to corresponding tracker classes
TRACKER_MAP = {"bytetrack": BYTETracker, "botsort": BOTSORT, "posetrack": PoseTracker}
...
def on_predict_start(predictor: object, persist: bool = False) -> None:
    ...
    if cfg.tracker_type not in {"bytetrack", "botsort", "posetrack"}:
        raise AssertionError(f"Only 'bytetrack', 'botsort' and 'posetrack' are supported for now, but got '{cfg.tracker_type}'")

    predictor._feats = None  # reset in case used earlier
    if hasattr(predictor, "_hook"):
        predictor._hook.remove()
    if cfg.tracker_type in ["botsort", "posetrack"] and cfg.with_reid and cfg.model == "auto":
